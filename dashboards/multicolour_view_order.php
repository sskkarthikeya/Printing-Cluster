...
if (!isset($_GET['id'])) {
    header("Location: multicolour_dashboard.php");
    exit;
}

$job_id = $_GET['id'];
$sql = "SELECT * FROM job_sheets WHERE id = ? AND multicolour = 1";
...
<head>
    <meta charset="UTF-8">
    <title>View Multicolour Order</title>
...
<body>
    <div class="navbar">
        <h2 class="brand">View Multicolour Order</h2>
        <div class="nav-buttons">
            <button class="back-btn" onclick="location.href='multicolour_dashboard.php'">Back to Multicolour</button>
        </div>
    </div>
    ...
    <div class="order-details">
        ...
        <p><strong>File:</strong> 
            <?php if ($job['file_path']): ?>
                <button class="download-btn" onclick="location.href='multicolour_view_order.php?id=<?= $job['id'] ?>&download=true'">Download All</button>
            <?php else: ?>
                No file
            <?php endif; ?>
        </p>
