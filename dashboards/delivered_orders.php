...
if (isset($_GET['download_file']) && isset($_GET['job_id'])) {
    ...
    $sql = "SELECT file_path FROM job_sheets WHERE id = ? AND completed_delivery = 1";
    ...
    if ($row && in_array($file_path, explode(',', $row['file_path']))) {
        if (file_exists($file_path) && is_readable($file_path)) {
            ...
            echo "<script>alert('File not found or inaccessible!'); window.location.href='delivered_orders.php';</script>";
        }
    } else {
        echo "<script>alert('Invalid file or job!'); window.location.href='delivered_orders.php';</script>";
    }
}

// Fetch completed Delivery orders, newest first
$sql = "SELECT * FROM job_sheets WHERE completed_delivery = 1 ORDER BY id DESC";
...
<body>
    <div class="navbar">
        <h2 class="brand">Delivered Orders</h2>
        <div class="nav-buttons">
            <button onclick="location.href='delivery.php'">Home</button>
    ...
    <div class="main-container">
        <div class="user-container">
            <h2>Completed Delivery Orders</h2>
                                ...
                                <td>
                                    ...
                                    if (!empty($db_files)): ?>
                                        <?php foreach ($db_files as $index => $file): ?>
                                            <a href="delivered_orders.php?download_file=<?= urlencode($file) ?>&job_id=<?= $row['id'] ?>" class="download-link">Download <?= $index + 1 ?></a>
                                        <?php endforeach; ?>
                                    <?php else: ?>
                                        No file
                                    <?php endif; ?>
