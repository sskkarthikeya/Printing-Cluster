...
$job_id = $_GET['id'];
$sql = "SELECT * FROM job_sheets WHERE id = ? AND completed_delivery = 0";
...
<body>
    <div class="navbar">
        <h2 class="brand">View Delivery Order</h2>
        <div class="nav-buttons">
            <button class="back-btn" onclick="location.href='delivery.php'">Back to Delivery</button>
        </div>
    </div>
    ...
    <div class="order-details">
        ...
        <p><strong>File:</strong> 
            <?php if ($job['file_path']): ?>
                <button class="download-btn" onclick="location.href='delivery_view_order.php?id=<?= $job['id'] ?>&download=true'">Download All</button>
            <?php else: ?>
                No file
            <?php endif; ?>
        </p>
