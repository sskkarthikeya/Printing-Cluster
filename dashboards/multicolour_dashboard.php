...
if (isset($_GET['complete_id'])) {
    ...
    $sql = "SELECT file_path FROM job_sheets WHERE id = ? AND multicolour = 1 AND completed_multicolour = 0";
    ...
    if ($job && ($job['file_path'] || !empty($new_file_paths))) {
        ...
        $sql = "UPDATE job_sheets SET completed_multicolour = 1, file_path = ? WHERE id = ? AND multicolour = 1";
        $stmt = $conn->prepare($sql);
        ...
        if ($stmt->execute()) {
            ...
            echo "<script>alert('Order completed in Multicolour and sent to Delivery!'); window.location.href='multicolour_dashboard.php';</script>";
        } else {
...
if (isset($_GET['download_file']) && isset($_GET['job_id'])) {
    ...
    $sql = "SELECT file_path FROM job_sheets WHERE id = ? AND completed_multicolour = 0";
    $stmt = $conn->prepare($sql);
    $stmt->bind_param("i", $job_id);
    $stmt->execute();
    $result = $stmt->get_result();
    ...
    if ($row && in_array($file_path, explode(',', $row['file_path']))) {
        if (file_exists($file_path) && is_readable($file_path)) {
            ...
            echo "<script>alert('File not found or inaccessible!'); window.location.href='multicolour_dashboard.php';</script>";
        }
    } else {
        echo "<script>alert('Invalid file or job!'); window.location.href='multicolour_dashboard.php';</script>";
    }
}

// Fetch active Multicolour orders, newest first
$sql = "SELECT * FROM job_sheets WHERE multicolour = 1 AND completed_multicolour = 0 AND (completed_ctp = 1 OR ctp = 0) ORDER BY id DESC";
...
<head>
    <meta charset="UTF-8">
    <title>Multicolour Dashboard</title>
...
<body>
    <div class="navbar">
        <h2 class="brand">Multicolour Dashboard</h2>
        <div class="nav-buttons">
            <button onclick="location.href='multicolour_dashboard.php'">Home</button>
            <button onclick="location.href='multicolour_completed_orders.php'">Completed Orders</button>
    ...
    <div class="main-container">
        <div class="user-container">
            <h2>Welcome, Multicolour User!</h2>
            <h3>Active Multicolour Orders</h3>
                ...
                <table>
                    ...
                    <tbody>
                        <?php while ($row = $result->fetch_assoc()): ?>
                            <tr <?php echo ($row['ctp'] == 1 && $row['multicolour'] == 1) ? 'class="from-ctp"' : ''; ?>>
                                <td><?= $row['id'] ?></td>
                                ...
                                <td>
                                    ...
                                    if (!empty($db_files)): ?>
                                        <?php foreach ($db_files as $index => $file): ?>
                                            <a href="multicolour_dashboard.php?download_file=<?= urlencode($file) ?>&job_id=<?= $row['id'] ?>" class="download-link">Download <?= $index + 1 ?></a>
                                        <?php endforeach; ?>
                                    <?php endif; ?>
                                ...
                                <td>
                                    ...
                                    <?php if ($row['ctp'] == 1 && $row['multicolour'] == 1): ?>
                                ...
                                <td>
                                    ...
                                    <button class="complete-btn" onclick="if(confirm('Mark this order as completed?')) location.href='multicolour_dashboard.php?complete_id=<?= $row['id'] ?>'" 
            ...
            <?php else: ?>
                <p>No active Multicolour orders found.</p>
            <?php endif; ?>
        </div>
    </div>
</body>
