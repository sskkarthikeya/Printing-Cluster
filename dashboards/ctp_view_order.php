...
if (!isset($_GET['id'])) {
    header("Location: ctp_dashboard.php");
    exit;
}

$job_id = $_GET['id'];
$sql = "SELECT * FROM job_sheets WHERE id = ? AND ctp = 1";
...
<body>
    <div class="navbar">
        <h2 class="brand">View CTP Order</h2>
        <div class="nav-buttons">
            <button class="back-btn" onclick="location.href='ctp_dashboard.php'">Back to CTP</button>
        </div>
    </div>
    ...
    <div class="order-details">
        ...
        <p><strong>File:</strong> 
            <?php if ($job['file_path']): ?>
                <button class="download-btn" onclick="location.href='ctp_view_order.php?id=<?= $job['id'] ?>&download=true'">Download All</button>
            <?php else: ?>
                No file
            <?php endif; ?>
        </p>
